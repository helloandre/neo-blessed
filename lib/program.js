...
Program.prototype._listenInput = function() {
  ...
  this.input.on('keypress', this.input._keypressHandler = function(ch, key) {
    ...
    // support key chords in the form of C-x x
    if (this._chordTimeout) {
      key.chordTrigger = this._chordTrigger;
      name = [this._chordTrigger.full, name].join(' ');
      clearTimeout(this._chordTimeout);
      delete this._chordTimeout;
      delete this._chordTrigger;
    } else if (key.ctrl || key.meta || key.shift) {
      // only support key chords that start with a special character
      this._chordTrigger = key;
      // wait up to 1000ms for next key to appear
